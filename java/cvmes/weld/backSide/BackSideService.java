 ...
 * 焊装后围计划发送服务
 */
public class BackSideService extends AbstractSubServiceThread {

    // 后围内存地址组编码
    private final String groupMemoryCode = "backside.around";

    @Override
    public void initServiceCode() {
        // 服务编码
        this.strServiceCode = "BackSideService";
    }
    ...
    private String DealHeader() {
        ...
        if (list_memory_write.size() != 0) {
            msg = String.format("后围写表内存地址组【%s】存在未处理指令，请检查OPC通讯是否正常", groupMemoryCode);
        ...
        if (list_memory_read.size() != 1) {
            msg = String.format("读取后围内存地址组【%s】配置错误，请检查配置", groupMemoryCode);
            Log.Write(strServiceCode, LogLevel.Error, msg);
            return msg;
        }
        ...
        if (automatism == null) {
            msg = String.format("后围读表内存地址组【%s】自动标记状态为空，请检查自动标记状态", groupMemoryCode);
        ...
        if ("1".equals(automatism)) {
            msg = String.format("后围读表内存地址组【%s】PLC处于离线状态，不下发计划", groupMemoryCode);
            Log.Write(strServiceCode, LogLevel.Information, msg);
            return msg;
        }
    ...
    private Record executeOperation(String strServiceCode, Record memoryRead) {
        ...
        //1 获取焊装后围plc计划队列
        List<Record> list_manual_plan = Db.find(ComFun.getBackSidePlan());
        ...
        msg = String.format("焊装后围,发送计划,下达控制命令字[%s],钢码号为[%s]",
                "1",
                bodySteelCode);
        Log.Write(strServiceCode, LogLevel.Information, msg);
        recordMsg.set("msg", msg);
    ...
    private Record reset(String strServiceCode, Record memoryRead) {
        ...
        if (!bodySteelCode.equals(steelCode)) {
            msg = String.format("焊装后围检验钢码号失败：计划队列的刚码号[%s],内存读表的钢码号[%s]", steelCode, bodySteelCode);
            Log.Write(strServiceCode, LogLevel.Error, msg);
            recordMsg.set("msg", msg);
            return recordMsg;
        }
        ...
        boolean isSucess = Db.tx(new IAtom() {
            ...
            public boolean run() throws SQLException {
                String msg = "";
                //2.1 修改完成状态
                int update = Db.update("UPDATE  T_INF_TO_WELD_BSIDE SET DEAL_STATUS='1',DEAL_TIME=sysdate WHERE CARBODY_CODE=?", bodySteelCode);
                if (update <= 0) {
                    msg = String.format("焊装后围计划处理失败:内存读表的钢码号:[%s]", bodySteelCode);
                    Log.Write(strServiceCode, LogLevel.Error, msg);
                    recordMsg.set("msg", msg).set("error", false);
                    return false;
                }
                //2.2
                msg = String.format("焊装后围计划处理成功:内存读表的钢码号:[%s]", bodySteelCode);
                Log.Write(strServiceCode, LogLevel.Information, msg);
                ...
                msg = String.format("焊装后围清空下位数据,下达控制命令字[%s]", "0");
                Log.Write(strServiceCode, LogLevel.Information, msg);
