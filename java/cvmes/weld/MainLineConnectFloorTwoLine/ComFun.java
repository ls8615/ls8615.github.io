...
public class ComFun {
     ...
     * 获取焊装地板二线与主线交接点计划任务
     */
    public static String getMainLinePlanQueue() {
        StringBuffer sql = new StringBuffer(256);
        sql.append("SELECT M.* FROM  (SELECT T.STEEL_CODE,");
        sql.append("       RPAD(T.ROBOT_CODE, 60, '0') ROBOT_CODE,");
        sql.append("        T.CARBODY_FIGURE_CODE,");
        sql.append("       T.LINE_CODE");
        sql.append("  FROM T_INF_TO_WELD_MSIDE T");
        sql.append(" WHERE T.DEAL_STATUS = 0");
        sql.append("  ORDER BY T.SEQ_NO ASC) M");
     ...
     * 获取修改处理状态sql
     */
    public static String updateMainLinePlanStatus() {
        StringBuffer sql = new StringBuffer(256);
        sql.append("UPDATE T_INF_TO_WELD_MSIDE");
        sql.append("   SET DEAL_STATUS = '1', DEAL_TIME = SYSDATE");
        sql.append(" WHERE STEEL_CODE = ?");
