 ...
 * 焊装FB010计划离线回线服务
 */
public class OffAndReturnLinePlanFb220Service {

    //子服务入口
    public void run() {
        Thread thread = new Thread(new OffAndReturnLinePlanFb220Thread());
        thread.start();
    }
     ...
     * 子服务线程
     */
    class OffAndReturnLinePlanFb220Thread extends Thread {

        // 服务编码
        private final String strServiceCode = "OffAndReturnFb220Service";

        // Fb220离线回线内存地址组编码
        private final String groupMemoryCode = "plan.offline.returnline.fb220";
        ...
        private String DealHeader() {
            ...
            if (list_memory_write.size() != 0) {
                msg = String.format("Fb220计划离线回线写表内存地址组【%s】存在未处理指令，请检查OPC通讯是否正常", groupMemoryCode);
            ...
            if (list_memory_read.size() != 1) {
                msg = String.format("读取Fb220计划离线回线内存地址组【%s】配置错误，请检查配置", groupMemoryCode);
                Log.Write(strServiceCode, LogLevel.Error, msg);
                return msg;
            }
            ...
            if (automatism==null) {
                msg = String.format("Fb220计划离线回线读表内存地址组【%s】自动标记状态为空，请检查自动标记状态", groupMemoryCode);
            ...
            if ("1".equals(automatism)) {
                msg = String.format("Fb220计划离线回线读表内存地址组【%s】PLC处于离线状态，不下发计划", groupMemoryCode);
                Log.Write(strServiceCode, LogLevel.Information, msg);
                return msg;
            }
        ...
        private Record successPlan(String strServiceCode, Record memoryRead) {
            ...
            msg = String.format("焊装FB220计划离线回线计划发送成功,下达控制命令字[%s]", "4");
            Log.Write(strServiceCode, LogLevel.Information, msg);
        ...
        private Record executeOperation(String strServiceCode, Record memoryRead) {
            ...
            //1.获取Fb220离线回线计划队列
            List<Record> list_manual_plan = Db.find(ComFun.getOffAndReturnLinePlanFB220Plan());
            if (list_manual_plan == null || list_manual_plan.size() == 0) {
                msg = "没有获取到焊装Fb220离线回线计划任务";
            ...
            if (bodySteelCode.length() == 0 || bodySteelCode.equals("null")) {
                msg = "获取焊装FB220计划离线回线队列车身钢码号的值错误";
                Log.Write(strServiceCode, LogLevel.Warning, msg);
                recordMsg.set("msg", msg);
                return recordMsg;
            }
            ...
            //2.2发Fb220工位的离线计划时，标记改为1，写入计划类型、车身钢码
            String logicValue;
            ...
            if (("0").equals(planType)) {
                ...
                if (preProductionCode.length() == 0 || preProductionCode.equals("null")) {
                    msg = "获取焊装FB220计划离线回线队列前置车身钢码号生产编码的值错误";
                    Log.Write(strServiceCode, LogLevel.Warning, msg);
                    recordMsg.set("msg", msg);
                    return recordMsg;
                }
                ...
                if (k_stamp_id.length() == 0 || k_stamp_id.equals("null")) {
                    msg = "获取需求产品表焊装FB220计划离线回线队列前置车身钢码号的值错误";
                    Log.Write(strServiceCode, LogLevel.Warning, msg);
                    recordMsg.set("msg", msg);
                    return recordMsg;
                }
            ...
            msg = String.format("焊装FB220计划离线回线,发送计划,下达控制命令字[%s],计划类型为[%s],钢码号为{%s},前置车身钢码号为[%s]",
                    logicValue,
                    planType,
                    bodySteelCode,
                    k_stamp_id);
        ...
        private Record reset(String strServiceCode, Record memoryRead) {
            ...
            if (steelCode.length() == 0 || steelCode.equals("null")) {
                msg = "获取焊装Fb220离线回线队列计划车身钢码号的值错误";
                Log.Write(strServiceCode, LogLevel.Warning, msg);
                recordMsg.set("msg", msg);
                return recordMsg;
            }
            ...
            if (!bodySteelCode.equals(steelCode)) {
                msg = String.format("焊装Fb220离线回线检验钢码号失败：计划队列的刚码号[%s],内存读表的钢码号[%s]", steelCode, bodySteelCode);
                Log.Write(strServiceCode, LogLevel.Error, msg);
                recordMsg.set("msg", msg);
                return recordMsg;
            }
            ...
            switch (planType) {
                case "1":
                    ...
                    boolean isSucess = Db.tx(new IAtom() {
                        ...
                        public boolean run() throws SQLException {
                            ...
                            if (update <= 0) {
                                msg = String.format("焊装Fb220离线计划处理失败:内存读表的钢码号:[%s],生产编码:[%s]", bodySteelCode,bodySteelProdectCode);
                                Log.Write(strServiceCode, LogLevel.Error, msg);
                                recordMsg.set("msg", msg).set("error", false);
                                return false;
                            }
                            msg = String.format("焊装Fb220离线回线计划处理成功:内存读表的钢码号:[%s],生产编码:[%s]", bodySteelCode,bodySteelProdectCode);
                            Log.Write(strServiceCode, LogLevel.Information, msg);
                            //2 清空写表
                            clearWrite();
                            msg = String.format("焊装Fb220离线回线清空下位数据,下达控制命令字[%s]", "0");
                            Log.Write(strServiceCode, LogLevel.Information, msg);
                ...
                case "2":
                    ...
                    if (preProductionCode.length() == 0 || preProductionCode.equals("null")) {
                        msg = "获取焊装Fb220离线回线队列计划前置车身生产编码的值错误";
                        Log.Write(strServiceCode, LogLevel.Warning, msg);
                        recordMsg.set("msg", msg);
                        return recordMsg;
                    }
                    ...
                    if (!productionCode.equals(preProductionCode)){
                        msg = String.format("焊装Fb220回线车检验前置钢码号生产编码失败：" +
                    ...
                    Db.tx(new IAtom() {
                        ...
                        public boolean run() throws SQLException {
                            ...
                            if (update <= 0) {
                                msg = String.format("焊装Fb220回线计划处理失败:内存读表的钢码号:[%s],前置车身钢码号为[%s]:", bodySteelCode,loopForward);
                                Log.Write(strServiceCode, LogLevel.Error, msg);
                                recordMsg.set("msg", msg).set("error", false);
                                return false;
                            }
                            msg = String.format("焊装Fb220离线回线计划处理成功:内存读表的钢码号:[%s],前置车身钢码号为[%s]:", bodySteelCode,loopForward);
                            Log.Write(strServiceCode, LogLevel.Information, msg);
                            //2.2发送成功，清空写表
                            clearWrite();
                            msg = String.format("焊装Fb220离线回线清空下位数据,下达控制命令字[%s]", "0");
