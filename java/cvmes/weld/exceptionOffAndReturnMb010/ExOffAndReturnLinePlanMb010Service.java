package cvmes.weld.exceptionOffAndReturnMb010;
 ...
 * 焊装异常离线回线服务
 */
public class ExOffAndReturnLinePlanMb010Service extends AbstractSubServiceThread {


    // Mb010异常离线回线内存地址组编码
    private final String groupMemoryCode = "mb010.exception.offline.returnline.plan";

    //工位编码
    private final String stationCode = "MB010";
    ...
    public void initServiceCode() {
        // 服务编码
        this.strServiceCode = "ExOffAndReturnMb010Service";
    }
    ...
    private String DealHeader() {
        ...
        if (list_memory_write.size() != 0) {
            msg = String.format("Mb010异常离线回线写表内存地址组【%s】存在未处理指令，请检查OPC通讯是否正常", groupMemoryCode);
        ...
        if (list_memory_read.size() != 1) {
            msg = String.format("读取Mb010异常离线回线内存地址组【%s】配置错误，请检查配置", groupMemoryCode);
            Log.Write(strServiceCode, LogLevel.Error, msg);
            return msg;
        }
        ...
        if (automatism == null) {
            msg = String.format("Mb010异常离线回线读表内存地址组【%s】自动标记状态为空，请检查自动标记状态", groupMemoryCode);
            Log.Write(strServiceCode, LogLevel.Information, msg);
            return msg;
        }

        //2.1.1.PLC在线状态，不做处理，结束运行
        if ("2".equals(automatism)) {
            msg = String.format("Mb010异常离线回线读表内存地址组【%s】PLC处于在线状态，不下发计划", groupMemoryCode);
            Log.Write(strServiceCode, LogLevel.Information, msg);
            return msg;
        }
        ...
        boolean isSucess = Db.tx(new IAtom() {
            ...
            public boolean run() throws SQLException {
                ...
                int update = Db.update(ComFun.insertExcReturnOffLinePlan(),
                        ...
                        stationCode);
                if (update <= 0) {
                    msg = String.format("焊装Mb010异常离线回线计划记录失败:内存读表的钢码号:[%s],计划类型为:[%s]", bodySteelCode, planType);
                    Log.Write(strServiceCode, LogLevel.Error, msg);
                ...
                //2.2 插入成功
                msg = String.format("焊装Mb010异常离线回线计划记录成功:内存读表的钢码号:[%s],前置车身钢码:[%s],计划类型为:[%s]", bodySteelCode, preBodySteelCode, planType);
                Log.Write(strServiceCode, LogLevel.Information, msg);
                ...
                msg = String.format("焊装Mb010异常离线回线,下达控制命令字[%s]", "3");
                Log.Write(strServiceCode, LogLevel.Information, msg);
