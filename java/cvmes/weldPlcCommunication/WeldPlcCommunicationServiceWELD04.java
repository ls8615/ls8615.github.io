 ...
 * 焊装副线-右围——PLC通讯服务
 */
public class WeldPlcCommunicationServiceWELD04 extends PlcSiemens {
    ...
    public String runBll(Record rec_service) throws Exception {
        ...
        //1.获取同步——右围计划
        ret = syncMemoryRightSide(dbCode);
     ...
     * @return
     */
    private Record syncMemoryRightSide(int dbCode) {

        String msg = "";
        Record recordMsg = new Record().set("isWrite", false).set("isWriteSucess", false).set("isReadSucess", false).set("msg", "");

        //内存地址组
        String groupMemoryCode = "rightside.around";

        int len = 90;

        //0.获取读表值
        List<Record> list_read = Db.find(getMemoryReadRightsideSql(), groupMemoryCode);
        if (list_read.size() != 1) {
        ...
        //1.获取写表未处理指令
        List<Record> list_write = Db.find(getMemoryWriteRightSideSql(), groupMemoryCode);
        ...
        if (list_write.size() == 1) {
            ...
            //1.2.1.获取写第一组数据，
            byte[] bytes = getWriteBytesFromRecordRight(list_write.get(0));
        ...
        //1.3.1.解析内存地址值
        Record recordRead = getReadRecordFromBytesRight(readBytes);

        //1.3.2.更新内存读表
        int ret = Db.update(getUpdateMemoryReadRightSql(),
     ...
     * @return
     */
    private Record getReadRecordFromBytesRight(byte[] readBytes) {
     ...
     * @return
     */
    private byte[] getWriteBytesFromRecordRight(Record memoryWrite) {
    ...
    /**
     * 获取焊装左侧围写表指令，参数：内存地址组
     *
     * @return
     */
    public String getMemoryReadRightsideSql() {
     ...
     * @return
     */
    public String getMemoryWriteRightSideSql() {
        StringBuffer sql = new StringBuffer(256);
        sql.append("SELECT T.ID,");
        sql.append("       T.LOGIC_VALUE,");
        sql.append("       T.AUTOMATISM,");
     ...
     * @return
     */
    public String getUpdateMemoryReadRightSql() {
        StringBuffer sql = new StringBuffer(256);
