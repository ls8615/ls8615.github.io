 ...
 * 焊装副线-后围——PLC通讯服务
 */
public class WeldPlcCommunicationServiceWELD02 extends PlcSiemens {
     ...
     * @return
     */
    private Record syncMemoryBackSide(int dbcode) {
        String msg = "";
        Record recordMsg = new Record().set("isWrite", false).set("isWriteSucess", false).set("isReadSucess", false).set("msg", "");

        //内存地址组
        String groupMemoryCode = "backside.around";

        int len = 90;

        //0.获取读表值
        List<Record> list_read = Db.find(getMemoryReadBacksideSql(), groupMemoryCode);
        if (list_read.size() != 1) {
        ...
        //1.获取写表未处理指令
        List<Record> list_write = Db.find(getMemoryWriteBacksideSql(), groupMemoryCode);
        ...
        if (list_write.size() == 1) {
            ...
            //1.2.1.获取写第一组数据，
            byte[] bytes = getWriteBytesFromRecordBack(list_write.get(0));
        ...
        //1.3.1.解析内存地址值
        Record recordRead = getReadRecordFromBytesBack(readBytes);

        //1.3.2.更新内存读表
        int ret = Db.update(getUpdateMemoryReadBackSql(),
     ...
     * @return
     */
    private Record getReadRecordFromBytesBack(byte[] readBytes) {
     ...
     * @return
     */
    private byte[] getWriteBytesFromRecordBack(Record memoryWrite) {
     ...
     * @return
     */
    public String getMemoryReadBacksideSql() {
     ...
     * @return
     */
    public String getMemoryWriteBacksideSql() {
        StringBuffer sql = new StringBuffer(256);
        sql.append("SELECT T.ID,");
        sql.append("       T.LOGIC_VALUE,");
        sql.append("       T.AUTOMATISM,");
     ...
     * @return
     */
    public String getUpdateMemoryReadBackSql() {
        StringBuffer sql = new StringBuffer(256);
