 ...
 * 焊装副线-左围——PLC通讯服务
 */
public class WeldPlcCommunicationServiceWELD03 extends PlcSiemens {
     ...
     * @return
     */
    private Record syncMemoryLeftSide(int dbCode) {

        String msg = "";
        Record recordMsg = new Record().set("isWrite", false).set("isWriteSucess", false).set("isReadSucess", false).set("msg", "");

        //内存地址组
        String groupMemoryCode = "leftside.around";

        int len = 90;

        //0.获取读表值
        List<Record> list_read = Db.find(getMemoryReadLeftsideSql(), groupMemoryCode);
        if (list_read.size() != 1) {
        ...
        //1.获取写表未处理指令
        List<Record> list_write = Db.find(getMemoryWriteLeftSideSql(), groupMemoryCode);
        ...
        if (list_write.size() == 1) {
            ...
            //1.2.1.获取写第一组数据，
            byte[] bytes = getWriteBytesFromRecordLeft(list_write.get(0));
        ...
        //1.3.1.解析内存地址值
        Record recordRead = getReadRecordFromBytesLeft(readBytes);

        //1.3.2.更新内存读表
        int ret = Db.update(getUpdateMemoryReadLeftSql(),
     ...
     * @return
     */
    private Record getReadRecordFromBytesLeft(byte[] readBytes) {
     ...
     * @return
     */
    private byte[] getWriteBytesFromRecordLeft(Record memoryWrite) {
     ...
     * @return
     */
    public String getMemoryReadLeftsideSql() {
     ...
     * @return
     */
    public String getMemoryWriteLeftSideSql() {
        StringBuffer sql = new StringBuffer(256);
        sql.append("SELECT T.ID,");
        sql.append("       T.LOGIC_VALUE,");
        sql.append("       T.AUTOMATISM,");
     ...
     * @return
     */
    public String getUpdateMemoryReadLeftSql() {
        StringBuffer sql = new StringBuffer(256);
