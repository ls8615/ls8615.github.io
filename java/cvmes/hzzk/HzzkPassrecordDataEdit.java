...
public class HzzkPassrecordDataEdit {
    public void run() {
        Thread thread = new Thread(new HzzkPassrecordDataEditThread());
        thread.start();
    }

    class HzzkPassrecordDataEditThread extends Thread {
        // 服务编码
        private final String strServiceCode = "HzzkPassrecordDataEdit";

        // 业务操作
        private void runBll(Record rec_service) {
            // 获取待处理的焊装中控过点记录数据（一次处理500条以内）
            List<Record> list = Db.find("select * from T_INF_TO_STAMPC_PASSREC where DEAL_STATUS = 0 and rownum <= 500");

            // 逐条处理焊装中控过点记录
            int ok = 0;
            int ng = 0;
            for (Record sub : list) {
                boolean flag = dealHzzkPassRecord(sub);
                if (flag) {
                    sub.set("DEAL_STATUS", 1).set("DEAL_TIME", new Date());
                    ok++;
                } else {
                    sub.set("DEAL_STATUS", 2).set("DEAL_TIME", new Date());
                    ng++;
                }
            ...
            // 更新服务信息
            if (list.size() > 0) {
                Db.update("update T_SYS_SERVICE set LAST_OPERATE_TIME=sysdate, LAST_OPERATE_INFO=? where SERVICE_CODE=?", String.format("共处理数据【%d】条，其中成功【%d】条，失败【%d】条", ok + ng, ok, ng), strServiceCode);
            }
        }

        private boolean dealHzzkPassRecord(Record rec) {
            try {
                // 获取生产线
                List<Record> list_line = Db.find("select * from T_MODEL_STATION where STATION_CODE = ?", rec.getStr("STATION_CODE"));
                if (list_line.size() != 1) {
                    Log.Write(strServiceCode, LogLevel.Error, String.format("ID【%s】，钢码号【%s】，工位【%s】的过点记录无法找到正确的生产线信息，请检查配置", rec.getStr("ID"), rec.getStr("STAMP_ID"), rec.getStr("STATION_CODE")));
                    return false;
                }
                ...
                List<Record> list_pass = Db.find("SELECT t2.ACTUAL_POINT_CODE, t3.PRODUCTION_CODE," +
                        " ? AS LINE_CODE, to_date(func_getnosys_workday(t1.PASS_TIME),'yyyy-mm-dd') AS WORK_DATE," +
                        " t3.DEMAND_PRODUCT_CODE AS PRODUCT_CODE, '1' AS PASSED_RECORD_TYPE, t1.PASS_TIME as PASSED_TIME," +
                        " (SELECT WORKSHOP_CODE FROM T_MODEL_LINE WHERE LINE_CODE=?) AS WORKSHOP_CODE" +
                        " FROM T_INF_TO_STAMPC_PASSREC t1" +
                        " LEFT JOIN T_MODEL_ACTUAL_POINT t2 ON t1.STATION_CODE = t2.STATION_CODE" +
                        " LEFT JOIN T_PLAN_DEMAND_PRODUCT t3 ON t1.STAMP_ID = t3.K_STAMP_ID" +
                        " WHERE t1.ID = ?", strLineCode, strLineCode, rec.getStr("ID"));
                if (list_pass.size() != 1) {
                    Log.Write(strServiceCode, LogLevel.Error, String.format("ID【%s】，钢码号【%s】，工位【%s】的过点记录匹配业务表数据失败，请检查数据", rec.getStr("ID"), rec.getStr("STAMP_ID"), rec.getStr("STATION_CODE")));
                    return false;
                }
                Record rec_pass = list_pass.get(0);
                if (rec_pass.getStr("ACTUAL_POINT_CODE") == null) {
                    Log.Write(strServiceCode, LogLevel.Error, String.format("ID【%s】，钢码号【%s】，工位【%s】的过点记录匹配业务表数据失败，实绩点编码不存在，请检查数据", rec.getStr("ID"), rec.getStr("STAMP_ID"), rec.getStr("STATION_CODE")));
                    return false;
                }
                if (rec_pass.getStr("PRODUCTION_CODE") == null) {
                    Log.Write(strServiceCode, LogLevel.Error, String.format("ID【%s】，钢码号【%s】，工位【%s】的过点记录匹配业务表数据失败，生产编码不存在，请检查数据", rec.getStr("ID"), rec.getStr("STAMP_ID"), rec.getStr("STATION_CODE")));
                    return false;
                }
                ...
                if (list_repeat.size() > 0) {
                    rec.set("DEAL_STATUS", 1).set("DEAL_TIME", new Date());
                    Db.update("T_INF_FROM_PAINTAVI_PASSRECORD", rec);
                    Log.Write(strServiceCode, LogLevel.Warning, String.format("过点记录数据重复，记录ID【%s】，钢码号【%s】，工位【%s】（实绩点【%s】），过点时间【%s】", rec.getStr("ID"), rec.getStr("STAMP_ID"), rec.getStr("STATION_CODE"), rec_pass.getStr("ACTUAL_POINT_CODE"), new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(rec_pass.getDate("PASSED_TIME"))));
                    return true;
                }
                ...
                e.printStackTrace();
                Log.Write(strServiceCode, LogLevel.Error, String.format("处理焊装中控过点记录失败，记录ID【%s】，钢码号【%s】，异常原因【%s】", rec.getStr("ID"), rec.getStr("STAMP_ID"), e.getMessage()));
                return false;
            }
        }
