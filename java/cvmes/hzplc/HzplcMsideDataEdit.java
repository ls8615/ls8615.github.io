...
public class HzplcMsideDataEdit {

    public void run() {
        Thread thread = new HzplcMsideDataEditThread();
        thread.start();
    }

    class HzplcMsideDataEditThread extends Thread {
        // 服务编码
        private final String strServiceCode = "HzplcMsideDataEdit";
        ...
        private void runBll(Record rec_service) {
            Db.tx(new IAtom(){
                ...
                public boolean run() throws SQLException {
                    ...
                    //判断焊接机器人的代码全部不为空
                    List<Object> rs = Db.query("SELECT E.K_STAMP_ID, F.PRODUCT_CODE, E.K_CARBODY_CODE,C.PRODUCTION_CODE " +
                    ...
                    List<Object[]> proCodeNullList = new ArrayList<>();
                    for (Object item: rs) {
                        Object items[] = (Object[]) item;
                        if(items[1] == null || items[1] == ""){
                            proCodeNullList.add(items);
                        }
                    }
                    if(proCodeNullList.size() > 0){
                        StringBuffer proCodeNullStr = new StringBuffer();
                        for(Object[] item : proCodeNullList){
                            proCodeNullStr.append("\t钢码号:").append(item[0])
                                    .append(",白车身图号:").append(item[2])
                                    .append(",生产编码:").append(item[3])
                                    .append("\n");
                        }
                        Log.Write(strServiceCode, LogLevel.Information, "指示点:"+indicationPointCode+",存在机器人焊接代码为空的数据。数据如下:\n[\n"+proCodeNullStr.toString()+"]。");
                        return true;
                    }
                    //删除已导入未处理的接口数据
                    ctn = Db.delete("DELETE FROM T_INF_TO_WELD_MSIDE A WHERE EXISTS ( " +
                            ...
                            "WHERE T1.SCHEDULING_PLAN_DATE = A.PLAN_DATE AND T2.LINE_CODE = A.LINE_CODE AND T.INDICATION_POINT_CODE = ? AND PRODUCTION_ORDER_STATUS = '0' AND PRODUCTION_ORDER_STATUS = '0' " +
                    ...
                    ctn = Db.update("INSERT INTO T_INF_TO_WELD_MSIDE(ID,STEEL_CODE,ROBOT_CODE,CARBODY_FIGURE_CODE,SEQ_NO,PLAN_DATE,DEAL_STATUS,DEAL_TIME,LINE_CODE,CREATE_TIME) " +
                            "SELECT SYS_GUID(),E.K_STAMP_ID, F.PRODUCT_CODE, E.K_CARBODY_CODE, C.SEQ_NO, A.PLAN_DATE,'0' AS DEAL_STATUS, SYSDATE, C.LINE_CODE, SYSDATE " +
                            "FROM T_CMD_PRODUCTION_ORDER A " +
                            "LEFT JOIN T_PLAN_SCHEDULING B ON(A.PLAN_NO = B.SCHEDULING_PLAN_CODE) " +
                            "LEFT JOIN T_PLAN_SCHEDULING_D C ON(C.SCHEDULING_PLAN_CODE = B.SCHEDULING_PLAN_CODE) " +
                            "LEFT JOIN T_PLAN_DEMAND_PRODUCT E on(E.PRODUCTION_CODE = C.PRODUCTION_CODE) " +
