...
public class HzplcRsideDataEdit {

    public void run() {
        Thread thread = new HzplcRsideDataEditThread();
        thread.start();
    }

    class HzplcRsideDataEditThread extends Thread {
        // 服务编码
        private final String strServiceCode = "HzplcRsideDataEdit";
        ...
        private void runBll(Record rec_service) {
            Db.tx(new IAtom(){
                ...
                public boolean run() throws SQLException {
                    ...
                    //删除已导入未处理的接口数据
                    ctn = Db.delete("DELETE FROM T_INF_TO_WELD_RSIDE A WHERE EXISTS ( " +
                    ...
                    ctn = Db.update("INSERT INTO T_INF_TO_WELD_RSIDE(ID,CARBODY_CODE, ROBOT_CODE, SEQ_NO, PLAN_TIME, DEAL_STATUS, LINE_CODE, DEAL_TIME) " +
                            "SELECT SYS_GUID(), E.K_STAMP_ID, F.PRODUCT_CODE, C.SEQ_NO, A.PLAN_DATE,'0' AS DEAL_STATUS, C.LINE_CODE, SYSDATE " +
