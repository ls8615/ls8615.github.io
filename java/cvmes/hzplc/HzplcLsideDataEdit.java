...
public class HzplcLsideDataEdit {

    public void run() {
        Thread thread = new HzplcLsideDataEditThread();
        thread.start();
    }

    class HzplcLsideDataEditThread extends Thread {
        // 服务编码
        private final String strServiceCode = "HzplcLsideDataEdit";
        ...
        private void runBll(Record rec_service) {
            Db.tx(new IAtom(){
                ...
                public boolean run() throws SQLException {
                    ...
                    //删除已导入未处理的接口数据
                    ctn = Db.delete("DELETE FROM T_INF_TO_WELD_LSIDE A WHERE EXISTS ( " +
                    ...
                    ctn = Db.update("INSERT INTO T_INF_TO_WELD_LSIDE(ID,CARBODY_CODE, ROBOT_CODE, SEQ_NO, PLAN_TIME, DEAL_STATUS, LINE_CODE, CREATE_TIME, DEAL_TIME) " +
