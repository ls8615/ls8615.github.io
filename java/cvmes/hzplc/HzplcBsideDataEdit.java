...
public class HzplcBsideDataEdit {

    public void run() {
        Thread thread = new HzplcBsideDataEditThread();
        thread.start();
    }

    class HzplcBsideDataEditThread extends Thread {
        // 服务编码
        private final String strServiceCode = "HzplcBsideDataEdit";
        ...
        private void runBll(Record rec_service) {
            Db.tx(new IAtom(){
                ...
                public boolean run() throws SQLException {
                    ...
                    //删除已导入未处理的接口数据
                    ctn = Db.delete("DELETE FROM T_INF_TO_WELD_BSIDE A WHERE EXISTS ( " +
                            ...
                            "WHERE T1.SCHEDULING_PLAN_DATE = A.PLAN_TIME AND T2.LINE_CODE = A.LINE_CODE AND T.INDICATION_POINT_CODE = ? AND T.PRODUCTION_ORDER_STATUS = '0' " +
                    ...
                    ctn = Db.update("INSERT INTO T_INF_TO_WELD_BSIDE(ID,CARBODY_CODE, ROBOT_CODE, SEQ_NO, PLAN_TIME, DEAL_STATUS, LINE_CODE, CREATE_TIME, DEAL_TIME) " +
