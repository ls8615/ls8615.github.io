...
public class TzaviPassrecordDataEditOnline {
    public void run() {
        Thread thread = new Thread(new TzaviPassrecordDataEditOnlineThread());
        thread.start();
    }

    class TzaviPassrecordDataEditOnlineThread extends Thread {
        // 服务编码
        private final String strServiceCode = "TzaviPassrecordDataEditOnline";

        // 业务操作
        private void runBll(Record rec_service) {
            // 处理涂装缓存前面的空撬
            Record recBB = Db.findFirst("select * from T_MODEL_ZONE WHERE ZONE_CODE='wbs10' AND PRODUCT_POS='1' AND PRODUCTION_CODE LIKE '%WBSBB%'");
            if (recBB != null) {
                int cnt = Db.findFirst("select count(1) as CNT from T_MODEL_ZONE where ZONE_CODE='wbs10'").getInt("CNT");
                Db.tx(new IAtom() {
                    public boolean run() throws SQLException {
                        // 移车
                        for (int i = 1; i < cnt; i++) {
                            Db.update("UPDATE T_MODEL_ZONE SET PRODUCTION_CODE =(SELECT PRODUCTION_CODE from t_model_zone where zone_code = 'wbs10' and product_pos = ?) WHERE ID = (SELECT ID from t_model_zone where zone_code = 'wbs10' and product_pos = ?)", i + 1, i);
                        }
                        Db.update("UPDATE T_MODEL_ZONE SET PRODUCTION_CODE = '' where id = (select id from t_model_zone where zone_code = 'wbs10' and product_pos = (select max(product_pos) from t_model_zone where zone_code = 'wbs10'))");
            ...
            for (Record sub : list) {
                ...
                if (flag) {
                    ...
                    Db.update("update T_SYS_SERVICE set LAST_OPERATE_TIME=sysdate, LAST_OPERATE_INFO=? where SERVICE_CODE=?", String.format("涂装上线记录处理失败导致卡车，ID【%s】，工位【%s】，钢码号【%s】", sub.getStr("ID"), sub.getStr("POSITION_NAME"), sub.getStr("FIN_NO")), strServiceCode);
                    return;
                }
            }
            ...
            // 更新服务信息
            if (list.size() > 0) {
                Db.update("update T_SYS_SERVICE set LAST_OPERATE_TIME=sysdate, LAST_OPERATE_INFO=? where SERVICE_CODE=?", String.format("成功处理涂装上线记录【%d】条", ok), strServiceCode);
            }
        }
