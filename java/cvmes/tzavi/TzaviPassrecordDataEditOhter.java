...
public class TzaviPassrecordDataEditOhter {
    public void run() {
        Thread thread = new Thread(new TzaviPassrecordDataEditOhterThread());
        thread.start();
    }

    class TzaviPassrecordDataEditOhterThread extends Thread {
        // 服务编码
        private final String strServiceCode = "TzaviPassrecordDataEditOhter";

        // 业务操作
        private void runBll(Record rec_service) {
            // 获取待处理的涂装AVI过点记录数据（一次500条以内）
            List<Record> list = Db.find("select * from T_INF_FROM_PAINTAVI_PASSRECORD where DEAL_STATUS = 0 and POSITION_NAME!=? and POSITION_NAME!=? and rownum<=500 ORDER BY MOVE_TIME", rec_service.getStr("SERVICE_PARA1_VALUE"), rec_service.getStr("SERVICE_PARA2_VALUE"));
            ...
            for (Record sub : list) {
                ...
                if (flag) {
                    ...
                    Db.update("update T_INF_FROM_PAINTAVI_PASSRECORD set DEAL_STATUS=2, DEAL_TIME=sysdate where ID=?", sub.getStr("ID"));
                    ng++;
                }
            }
