 ...
 *
 */
public class ZznjjWlTqInf extends AbstractSubServiceThread {
    private String msg;

    @Override
    public void initServiceCode() {
        this.strServiceCode = "ZznjjWlTqInf";
    }
    ...
    public String runBll(Record rec_service) throws Exception {
        ...
        List<Record> list = Db.find("select * from T_INF_TO_WHEEL_TORQUE where DEAL_STATUS = '0' and rownum <= 1500");
        if (list == null || list.size() == 0) {
            return msg;
        }

        List<Map<String, Object>> list_wltq = new ArrayList<Map<String, Object>>();
        for (Record sub : list) {
            ...
            map.put("FrontWheelTorque", sub.getStr("FRONT_WHEEL_TORQUE"));
            map.put("RearWheelTorque", sub.getInt("REAR_WHEEL_TORQUE"));
            list_wltq.add(map);
        }

        EntityWebServicePara entity = new EntityWebServicePara();
        entity.setKey(rec_service.getStr("SERVICE_PARA2_VALUE"));
        entity.setData(list_wltq);
        ...
        try {
            ...
            String str = (String) wsc.invoke("GetTireTorque", new String[]{"_para"}, new Object[]{json});
            ...
            if (errcode == 0) {
                // 成功
                for (Record sub : list) {
                    Db.update("update T_INF_TO_WHEEL_TORQUE set DEAL_STATUS = '1', DEAL_TIME = sysdate where ID = ?", sub.getStr("ID"));
                }
